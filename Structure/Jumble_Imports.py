...
class mouse_obj(gen_obj):
    def __init__(i, title = None):
        super().__init__(title)
        i.append({'x': 0, 'y': 0, 'm1': 0, 'm2': 1, 'm3': 2, 'n1': 0, 'n2': 1, 'n3': 2, 't1': 0, 't2': 1, 't3': 2, 't1_v': 0, 't2_v': 1, 't3_v': 2, 'mouse_visible': 1})
class image(gen_obj):
...
class images_obj(gen_obj):
    ...
    def generate(i, screen, mouse = None, UI = None):
        ...
        for this in i.attr_valuelist:
            ...
            if not mouse == None:
                ...
                if this.selected and this in i.selected: this.append({'x_pos': mouse.x - this.x_diff, 'y_pos': mouse.y - this.y_diff})
                if not UI == None:
                    ...
                    if this.title in i.UI_dict and i.UI_dict[this.title]:
                        nullvalue = UI.image_window(screen, mouse, this, i)
                        if nullvalue == 'exit':
...
def malleable_text(screen, x, y, image, image_attr_list, size = 25):
    for this in image_attr_list:
        message_display(screen, this + ': ' + str(image.attr_dict[this]), x, y + image_attr_list.index(this)*int(size*0.75), size, (255, 255, 255))
    ...
    def retrieve(i, path = 'Structure/UI/'):
        ...
        for this in os.listdir(path):
            if this[-4:] == ".jpg" or this[-4:] == ".png":
                ...
                i.append({this[:-4]: parsed_image})
    def image_window(i, screen, mouse, image, images):
        ...
        if i.right_select[image.title]: i.x_len[image.title] = mouse.x - i.x_pos[image.title]
        if i.x_len[image.title] < 100: i.x_len[image.title] = 100
        if i.right_select[image.title] and not mouse.m1: i.right_select[image.title] = 0
        ...
        if i.left_select[image.title]: nullvalue = i.x_pos[image.title]; i.x_pos[image.title] = mouse.x; i.x_len[image.title] -= i.x_pos[image.title] - nullvalue
        if i.x_len[image.title] < 100: i.x_len[image.title] = 100
        if i.left_select[image.title] and not mouse.m1: i.left_select[image.title] = 0
        ...
        if i.bottom_select[image.title]: i.y_len[image.title] = mouse.y - i.y_pos[image.title]
        if i.y_len[image.title] < 100: i.y_len[image.title] = 100
        if i.bottom_select[image.title] and not mouse.m1: i.bottom_select[image.title] = 0
        if windowbar.collidepoint(mouse.x, mouse.y) and mouse.t1: i.append({'x_diff': mouse.x - i.x_pos[image.title], 'y_diff': mouse.y - i.y_pos[image.title]}); i.selected.append(image.title)
        if image.title in i.selected and not mouse.m1: i.selected.remove(image.title); i.delete(['x_diff', 'y_diff'])
